...
function processGoogleRegister(inviteKey)
{
    var xhr = new XMLHttpRequest();
    ...
    xhr.open('POST', 'googleregister');
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    xhr.send("invite=" + encodeURIComponent(inviteKey));
}

function processGoogleLogin(googleUser, callback = null, redirect = true)
{
    ...
    xhr.onreadystatechange = function ()
    {
        if (xhr.readyState === 4) {
            if (xhr.status === 200 && xhr.status < 300)
            {
                ...
                else document.getElementById('txtHint').innerHTML = `OAuth: ${json.reason}<br><br>`;
            }
            else {
                document.getElementById('txtHint').innerHTML = "Failed to connect!<br><br>";
            }
